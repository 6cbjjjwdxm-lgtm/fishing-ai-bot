...
def extract_city_simple(query: str) -> str:
    """
    Достаёт локацию из фразы вида:
    "*какой клев завтра в Москве?" -> "Москве"
    "*клев завтра в Нижнем Новгороде" -> "Нижнем Новгороде"
    """
    q = (query or "").strip()

    m = re.search(r"\bв\s+([A-Za-zА-Яа-яЁё0-9\-\s]+)", q, flags=re.IGNORECASE)
    if not m:
        return ""
    ...
    stop = {
        ...
        "на", "по", "около", "районе", "области", "рф", "россии"
    }
    ...
    cleaned = cleaned[:3]  # "Нижний Новгород", "Санкт Петербург"
...
СЕЗОН (ОЧЕНЬ ВАЖНО):
- Если season = winter и пользователь НЕ сказал "открытая вода",
  то ВСЕ рекомендации давай под лёд:
  - Белая: мормышка, поплавок под лёд, мотыль/опарыш, аккуратная прикормка.
  - Хищник: жерлицы, балансир, вертикальная блесна.
  - НЕЛЬЗЯ предлагать джиг, твич, воблерные проводки, микроджиг и т.п.
- Летние снасти (джиг/проводки/воблеры) разрешены только если пользователь явно пишет "открытая вода" или "спиннинг по открытой".

Формат:
- Отвечай на русском.
- В прогнозе обязательно блоки "Белая рыба" и "Хищник".
